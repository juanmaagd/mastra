...
A model pack can prefer a different account for each model. In `/models`, select a pack, choose **Set subscription routing…**, select a model, then select an account. The routing preview shows the request order before you save it.

The preferred account is tried first. If it fails, Mastra Code tries the provider's remaining accounts in the order they were added. Exhausted accounts stay skipped for that model and thread on later messages. Select **Automatic** to use insertion order without a preferred account.

Preferences belong to the pack and resolved model. Two packs that use the same model can prefer different accounts. When a request hops to a fallback pack, that pack's own subscription routing applies.

#### Automatic account rotation

When a request fails on the active account, Mastra Code automatically rotates to the next account in the pool and retries:
...
- **Server errors and network outages** first exhaust the normal transient retry budget; if the provider still can't serve the request, the error surfaces (and, when a fallback model pack is configured, the turn hops to it).

Rotation starts with the account selected by the pack's subscription routing, then walks the remaining accounts in the order they were added. Each account is tried at most once per request. When every account in the pool has been tried, the error surfaces normally or the request moves to the configured fallback pack.
...
Custom model packs can carry an optional OM model. When creating or editing a pack in `/models`, the OM step lets you choose a model or skip it. OM models resolve in this order:
